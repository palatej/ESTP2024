...
The Henderson filter $\{\omega_i\}_{-p\le i \le p}$ is defined by minimizing $\sum_{i=-p}^p{(\Delta^3\omega_i)^2}$
under the constraints: $\sum_{i=-p}^p{\omega_i}=1$, $\sum_{i=-p}^p{i\omega_i}=0$ and $\sum_{i=-p}^p{i^2\omega_i}=0$
